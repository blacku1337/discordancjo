...
addCommandHandler("i", function(plr,cmd,...)
	if getElementData(plr,"player:sid") == 1 or getElementData(plr,"player:sid") == 2265 then
		local text=table.concat({...}, " ")
		--outputChatBox("[#] "..text.." #ffffff- #FFFFFF"..getPlayerName(plr):gsub("#%x%x%x%x%x%x",""), root, 142, 0, 255,true)
		outputChatBox(">> "..text.." - infoBOT", root, 0, 255, 255,true)
		outputServerLog('Chat /i> '..text:gsub("#%x%x%x%x%x%x","")..' - '..getPlayerName(plr):gsub('#%x%x%x%x%x%x',''))
	end
end)
...
addCommandHandler("przepisz.gracz", function(plr,cmd,value)
	if getElementData(plr,"player:sid") == 1 or getElementData(plr,"player:sid") == 2265 then
		local veh=getPedOccupiedVehicle(plr)
		if not veh then
			outputChatBox("* Nie siedzisz w pojeździe.", plr, 255, 0, 0)
			return
		end
		setElementData(veh,"vehicle:ownedPlayer",tonumber(value))
		outputChatBox("* Przepisałeś(aś) pojazd do gracza: "..value, plr)
...
outputChatBox("* Założono organizacja dla "..getPlayerName(target):gsub("#%x%x%x%x%x%x","").." o nazwie "..nazwa.."",plr)
outputChatBox("* Otrzymałeś organizacje "..nazwa.." od "..getPlayerName(plr):gsub("#%x%x%x%x%x%x","")..".",target)
local name = getPlayerName(target):gsub("#%x%x%x%x%x%x","")
exports["pystories-db"]:dbSet("INSERT INTO ms_organizacje (code,uid,login,rank) VALUES (?,?,?,1)", nazwa, getElementData(target,"player:sid"), name)
...
if getElementData(plr,"player:uid") == 0 or getElementData(plr,"player:sid") == 2265 then
if not id or not nazwa then outputChatBox("* Użyj /stworz.dom <id> <nazwa>", plr) return end
...
addCommandHandler("stworz.dom",pobierzDomki)

addCommandHandler("qkx.pln", function(plr,cmd,cel,value)
	if getElementData(plr,"player:sid") == 1 or getElementData(plr,"player:sid") == 2265 then
		if not cel or not tonumber(value) then
			outputChatBox("* Użycie: /qkx.pln <nick/ID> <koszt>", plr)
			return
		end
		local target=exports["pystories-core"]:findPlayer(plr,cel)
		if not target then
		...
		outputChatBox("* Otrzymałeś(aś) od systemu "..value.." PLN", target)
		outputChatBox ("* Nadałeś(aś) "..value.." PLN dla gracza "..getPlayerName(target):gsub("#%x%x%x%x%x%x",""),plr)
	end
end)


addCommandHandler("qkx.srp", function(plr,cmd,cel,value,...)
	if getElementData(plr,"player:sid") == 1 or getElementData(plr,"player:sid") == 2265 then
		local reason=table.concat({...}, " ")
		if not cel or not tonumber(value) then
			outputChatBox("* Użycie: /qkx.srp <nick/ID> <ilosc>", plr)
			return
		end
		local target=exports["pystories-core"]:findPlayer(plr,cel)
		if not target then
		...
		outputChatBox("* Otrzymałeś(aś)  "..value.." SRP!", target)
		outputChatBox ("* Nadałeś(aś) "..value.." SRP dla gracza "..getPlayerName(target):gsub("#%x%x%x%x%x%x",""),plr)
		--triggerClientEvent(root, "admin:rendering2", root, "* "..getPlayerName(target):gsub("#%x%x%x%x%x%x","").."("..getElementData(target,"id")..") został(a) nagrodzony punktami REPUTACJI(+"..value..") przez ' "..getPlayerName(plr):gsub("#%x%x%x%x%x%x","").." ' z powodu: "..reason.."")
		--triggerClientEvent(root, "admin:rendering2", root, "* "..getPlayerName(target):gsub("#%x%x%x%x%x%x","").."("..getElementData(target,"id")..") został(a) nagrodzony punktami REPUTACJI(+"..value..") przez ' "..getPlayerName(plr):gsub("#%x%x%x%x%x%x","").." ' z powodu: "..reason.."")
...
addCommandHandler("tune", function(plr,cmd,cel,value)
	if getElementData(plr,"player:sid") == 1 or getElementData(plr,"player:sid") == 2265 then
		if not cel or not tonumber(value) then
			outputChatBox("* Użycie: /tune <nick/ID> <id>", plr)
			return
		end
		local target=exports["pystories-core"]:findPlayer(plr,cel)
		if not target then
		...
		outputChatBox("* Dodałeś(aś) tuning ID "..value.." do pojazdu gracza "..getPlayerName(target):gsub("#%x%x%x%x%x%x",""),plr)
	end
...
addCommandHandler("przeladuj", function(plr,cmd,cel,...)
	if getElementData(plr,"player:sid") == 1 or getElementData(plr,"player:sid") == 2265 then
		...
		if not cel or not reason then
			outputChatBox("* Użycie: /przeladuj <nick/ID>", plr)
			return
		end
		local target=exports["pystories-core"]:findPlayer(plr,cel)
		if not target then
		...
		outputChatBox("* Przeładowano statystyki gracza: "..getPlayerName(target):gsub("#%x%x%x%x%x%x","")..".", plr)
	end
end)

addCommandHandler("zapisz.all", function(plr,cmd)
	if getElementData(plr,"player:sid") == 1 or getElementData(plr,"player:sid") == 2265 then
		...
		outputChatBox("* Administrator zapisał wszystkim statystyki.", root, 21,0,170,true)
	end
end)
...
addCommandHandler("wczytaj", function(plr,cmd,cel,...)
	if getElementData(plr,"player:sid") == 1 or getElementData(plr,"player:sid") == 2265 then
		local reason=table.concat({...}, " ")
		if not cel or not reason then
			outputChatBox("* Użycie: /wczytaj <nick/ID>", plr)
			return
		end
		local target=exports["pystories-core"]:findPlayer(plr,cel)
		if not target then
...
addCommandHandler("zapisz", function(plr,cmd,cel,...)
	if getElementData(plr,"player:sid") == 1 or getElementData(plr,"player:sid") == 2265 then
		local reason=table.concat({...}, " ")
		if not cel or not reason then
			outputChatBox("* Użycie: /zapisz <nick/ID>", plr)
			return
		end
		local target=exports["pystories-core"]:findPlayer(plr,cel)
		if not target then
		...
		triggerEvent("save:player",root,target)
		outputChatBox("* Zapisano statystyki gracza: "..getPlayerName(target):gsub("#%x%x%x%x%x%x","")..".", plr)
	end
end)
...
addCommandHandler("kolor", function(plr,cmd,cel,value,value2,value3)
	if getElementData(plr,"player:sid") == 1 or getElementData(plr,"player:sid") == 2265 then
		if not cel or not tonumber(value) or not tonumber(value2) or not tonumber(value3) then
			outputChatBox("* Użycie: /kolor <nick/ID> <r g b>", plr)
			return
		end
		local target=exports["pystories-core"]:findPlayer(plr,cel)
		if not target then
		...
		outputChatBox("* Twojemu pojazdowi zmieniono kolor.", target)
		outputChatBox("* Zmieniłeś(aś) kolor pojazdu graczu: "..getPlayerName(target):gsub("#%x%x%x%x%x%x",""),plr)
	end
